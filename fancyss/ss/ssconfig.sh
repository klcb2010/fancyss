...
mihomo_hook_check() {
    ...
    # AnyTLS = 9
    if [ "${ss_basic_type}" = "9" ]; then

        echo "[mihomo] AnyTLS detected"

        dbus set ss_mode=mihomo

        /jffs/.koolshare/scripts/ss_mihomo.sh start
...
kill_process() {
	...
	local ANYTLS_PID=$(ps | grep "anytls-go" | grep -v grep | awk '{print $1}')
	if [ -n "${ANYTLS_PID}" ];then
		echo_date "关闭anytls-go进程..."
		killall anytls-go
	fi
...
creat_hy2_json(){
	...
	# 默认情况：有 up/down 时 brutal，无 up/down 时 bbr: hub.glowp.xyz/https://github.com/XTLS/Xray-core/issues/5546
...
start_naive(){
	if [ -f "/koolshare/bin/naive" ];then
		...
		echo_date "hub.glowp.xyz/https://raw.githubusercontent.com/hq450/fancyss/3.0/fancyss/bin-${pkg_arch}/naive"
		echo_date ""
		echo_date "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
		echo_date ""
		close_in_five flag
...
start_tuic(){
	if [ -f "/koolshare/bin/tuic-client" ];then
		...
		echo_date "hub.glowp.xyz/https://raw.githubusercontent.com/hq450/fancyss/3.0/fancyss/bin-${pkg_arch}/tuic-client"
		echo_date ""
		echo_date "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
		echo_date ""
		close_in_five flag
...
start_anytls(){
	local server_addr=""
	local verify_arg="--insecure"
	
	# 强制使用安装到 bin 目录下的文件
	ANYTLS_BIN="/koolshare/bin/anytls-go"
	chmod +x "$ANYTLS_BIN"

	# 1. 变量清洗，防止不可见字符干扰
	local PASS=$(echo "${ss_basic_anytls_pass}" | tr -d '\r')
	local SNI=$(echo "${ss_basic_anytls_sni}" | tr -d '\r')
	local HOST=$(echo "${ss_basic_anytls_server}" | tr -d '\r')
	local PORT=$(echo "${ss_basic_anytls_port}" | tr -d '\r')

	# 2. 准备地址
	server_addr="${HOST}:${PORT}"

	# 3. 清理环境，防止端口占用
	killall anytls-go anytls-go anytls-client ipt2socks 2>/dev/null
	sleep 1
	...
	# 使用你手动测试成功的 -p (密码字符串) 和 -socks (监听端口)
	if [ -n "${SNI}" ]; then
		"$ANYTLS_BIN" -server "${server_addr}" \
			-p "${PASS}" \
			-socks 127.0.0.1:23456 \
			-sni "${SNI}" \
			${verify_arg} > /tmp/anytls_run.log 2>&1 &
	else
		"$ANYTLS_BIN" -server "${server_addr}" \
			-p "${PASS}" \
			-socks 127.0.0.1:23456 \
	...
	# 6. 进程检测（只要包含 anytls 就算运行成功）
	detect_running_status3 anytls 23456 1 force
}
...
detect_ip(){
	...
	if [ "${METHOD}" == "0" ];then
		...
		# 检测代理ip
		local SOCKS5_OPEN=$(netstat -nlpt 2>/dev/null|grep -w "23456"|grep -Eo "v2ray|xray|naive|tuic|anytls")
...
check_frn_public_ip(){
	...
	local SOCKS5_OPEN=$(netstat -nlp 2>/dev/null | grep -w "23456" | grep -Eo "v2ray|xray|naive|tuic|anytls" | head -n1)
	if [ -n "${SOCKS5_OPEN}" ];then
...
start_nat)
	# start on nat-start
	SOCKS5_OPEN=$(netstat -nlpt 2>/dev/null|grep -w "23456"|grep -Eo "v2ray|xray|naive|tuic|anytls")
