...
Tools:
  geotool node-tool sub-tool anytls-go xapi-tool ws-tool status-tool webtest-tool
...
tool_config() {
	...
	case "$tool" in
		...
		sub-tool)
			...
			;;
		anytls-go)
			printf '%s|%s|%s|%s\n' "tool/anytls-go" "binaries/anytls-go" "anytls-go" "x86_64 aarch64 armv5te armv7a armv7hf"
			;;
		xapi-tool)
