...
LOG_FILE="/jffs/ddns_force.log"  # 脚本内部日志，持久化

echo "[$(date '+%Y-%m-%d %H:%M:%S')] Script started - forcing DDNS update" >> "$LOG_FILE"
logger -t "DDNS_FORCE" "Script started - forcing DDNS update"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Script finished - DDNS force triggered" >> "$LOG_FILE"
logger -t "DDNS_FORCE" "Script finished - DDNS force triggered"
